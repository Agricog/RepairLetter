 ...
 * - RentShield header with case reference
...
export async function generateLetterPdf(options: LetterPdfOptions): Promise<Uint8Array> {
  ...
  // Logo text
  currentPage.drawText('RentShield', {
    x: MARGIN,
    y: yPos - 10,
    size: 18,
    font: helveticaBold,
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    // Disclaimer
    const disclaimer = 'This letter was generated by RentShield and does not constitute legal advice.';
...
export async function generateEvidencePackCover(
  ...
  // Title
  page.drawText('RentShield', { x: 50, y: 700, size: 28, font: helveticaBold, color: NAVY });
  ...
  // Disclaimer
  page.drawText(
    'This evidence pack is compiled from records stored securely by RentShield.',
